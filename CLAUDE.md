...
- **心智圖**（分頁 mind，唯讀視覺化）：`buildMindTree` 由 DB 生成「中心→類別→啟用產品→綁定包材(含首張照片)」放射樹；`mindLayout` 依葉數權重分配角度、半徑 R=[0,250,470,690]；`mindCollect`→ SVG cubic 連線＋絕對定位 .mnode；`mindSetup` 拖曳平移＋滾輪縮放（`mindT`/`mindApply`/`mindFit`/`mindZoom`）。點 product/pack 節點開 `productDetail`/`packDetail`。純前端，無外部函式庫。
